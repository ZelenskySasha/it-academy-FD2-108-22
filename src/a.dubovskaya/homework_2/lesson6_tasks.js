...
function getCount(str) {
    let vowelsCount = 0;
  let arraySym = ['a','o','u','e','i'];
    for (let i=0; i < str.length; i++) {
        const symbol = str[i];
      
      if (arraySym.includes(symbol)){
        vowelsCount += 1;
      }
...
function combine(inputs) {
    ...
    for(let i = 0; i < arguments.length; i++) {
        for ( let key in arguments[i]) {
          if(!objCombine[key]) {
                objCombine[key] = arguments[i][key];
          }
          else {
                objCombine[key] += arguments[i][key];
          }
...
const users = [
  ...
  let sortMaxToMin = users.sort((a,b) => {return b.age - a.age});
  let oldUser = sortMaxToMin[0];
  let youngUser = sortMaxToMin[sortMaxToMin.length - 1]
  ...
  const averageAge = (arr) => {
    ...
    return Math.round(age / arr.length);
  
  }
  console.log(averageAge(sortMaxToMin), oldUser, youngUser);
