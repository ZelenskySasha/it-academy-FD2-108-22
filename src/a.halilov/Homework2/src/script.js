...
console.log(sort());
  ...
  /* Задача 5 средний возраст */


function middleAge(){
      let middleAge = Math.round(users.reduce((acum, user) => acum + user.age, 0)/ users.length);
      return middleAge;
};
console.log(middleAge());
  ...
  /* Задача 6 Найти самого старшего/младшего пользователя */


function olderAndYoungerUsers (){
    let older = users.reduce((prev, cur) => prev > cur.age ? prev : cur.age);
    
    let younger = users.reduce((prev, cur) => prev < cur.age ? prev : cur.age);
    
    return `${older} and ${younger}`;
}
console.log(olderAndYoungerUsers());



function olderAndYoungerUsersElse(){
    let sorted = users.sort((a, b) => b.age - a.age);
    return [sorted[0], sorted[sorted.length-1]];
}

console.log(olderAndYoungerUsersElse());




/* Задача 7*** на вход получаете размеры двумерного массива n
