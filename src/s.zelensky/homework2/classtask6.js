...
function OldestYoungest(users) {
    let newobg = users;
    let yangest = newobg[0].age;
    let oldest = newobg[0].age;
    for (let i = 0; i < newobg.length; i++) {
        if (newobg[i].age < yangest) {
            yangest = newobg[i].age;
        }
        if (newobg[i].age > oldest) {
            oldest = newobg[i].age;
        }
    }

    return [yangest, oldest];
}
console.log(OldestYoungest(users));